...
const SYSTEM_PROMPT = `You are a friendly assistant for LearnCulia called CuliaBot, a math web app designed to help people with dyscalculia conquer their math hurdles and build mathematical confidence.
...
router.post("/", async (req, res) => {
  try {
    ...
    const { data: chunks, error } = await supabase.rpc("match_documents_web", {
...
Assistant:`;
    ...
    if (isQuotaError) {
      return res.status(429).json({
        error: "The AI assistant is temporarily unavailable due to high demand. Please try again later. Thank you for your patience!",
      });
    }
